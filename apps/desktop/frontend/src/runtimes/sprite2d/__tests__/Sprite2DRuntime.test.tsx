...
import {
  Sprite2DRuntime,
  type Sprite2DProjection,
} from "../Sprite2DRuntime";
...
const projection = (overrides: Partial<Sprite2DProjection> = {}): Sprite2DProjection => ({
  type: "avatar.directive",
  runtime: "sprite2d",
  directive: { state: "idle", emotion: "neutral", speaking: false },
  frames: ["/static/characters/octomimic/idle/0.png", "/static/characters/octomimic/idle/1.png"],
  fps: 10,
  loop: true,
  ...overrides,
});
...
beforeEach(() => {
  _resetCache();
  // @ts-expect-error -- swap Image globally for the test.
...
describe("Sprite2DRuntime", () => {
  ...
  it("wraps when loop is true", () => {
    render(<Sprite2DRuntime projection={projection({ fps: 10, loop: true })} />);
  ...
  it("clamps at the last frame when loop is false", () => {
    render(<Sprite2DRuntime projection={projection({ fps: 10, loop: false })} />);
    vi.advanceTimersByTime(110); // frame 1
