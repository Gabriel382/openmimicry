...
**Compatible character swap-in-place** — selecting a character derives its
renderer from `pack.yaml` (`sprite2d` → Sprite2D, `vrm`/`gltf` → Three.js),
loads the candidate atomically, and persists the pair. Per-pack 3D framing and
animation speed preview immediately and survive restart.
...
Claude CLI and PicoClaw remain separate, optional installations. Subscription
mode uses the user's existing local Claude login; API mode is an explicit
alternative.
...
# Three.js avatar with the ready-to-run bundled Octomimic VRM
...
- **Bundled VRM test model**:
  [`characters/octomimic_vrm/`](characters/octomimic_vrm/) — CC0 VRM 1.0
  with embedded listening, thinking, speaking, emotion, and gesture clips.
...
- **v1.8 integrated architecture**: [`docs/design/OpenMimicry-v1.8.0-Integrated-Architecture-Appendix.md`](docs/design/OpenMimicry-v1.8.0-Integrated-Architecture-Appendix.md)
- **v1.8 local agents and projects**: [`docs/local_agents_and_projects.md`](docs/local_agents_and_projects.md)
- **Tasks + local Claude subscription quickstart**: [`docs/TASKS_AND_CLAUDE.md`](docs/TASKS_AND_CLAUDE.md)
